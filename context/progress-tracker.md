...
- Unit 13 (`keymap-source-of-truth`) — keymaps populated, standards rule written, v0.54.1 staged. Foundation commit for Unit 14.
- Unit 14 (`migrate-msg-string-to-key-matches`) — next: replace the `msg.String()` switch in `update_release.go:374` and the other main-matchers in `update_commit.go` / `update_writing.go`, then refactor the `?` popup to introspect the active KeyMap.

## Next Up

- After Units 13 + 14 land: merge `feat/keymatches-migration` → `main`, tag + GitHub release.
