...
- `feat/agent-cli-improvements` — branch-level umbrella in `context/specs/15-agent-cli-improvements.md` (8 units across two axes: CLI ergonomics + branch/release messages).
  ...
  - Unit 17 (`ai merge`): shipped as v0.58.0, commit `ce113af`.
  - Unit 18 (`ai release`): shipped as v0.59.0, commit `e425e9b`. Drafts only — `ai release publish` is a follow-up.
  - Unit 19 (`ai link-commit`): code-complete, awaiting paired skill update.
  - Items 3, 5, 6: spec-on-demand per option B.
...
- 2026-05-27 — **Unit 19 (`ai link-commit`) code-complete** on `feat/agent-cli-improvements`. Schema migration adds `commit_hash TEXT NOT NULL DEFAULT ''` to `commits` (idempotent via existing duplicate-column guard). New `commitcraft ai link-commit --id <draft> --hash <git>` writes the hash; `commitcraft ai show --commit <hash>` looks up by prefix (≥4 chars, ambiguous match returns `ambiguous_hash` with candidate ids). New storage methods `LinkCommitHash` + `GetCommitsByHashPrefix`; new git helper `ResolveCommitHashAt(workspace, rev)`. `commitJSON` envelope gains `commit_hash` (`omitempty`). End-to-end dogfood: linked draft 993 → c79dfac (the `ai context` commit), then `ai show --commit c79dfac` returned the draft verbatim. Overwrite warning fired correctly when re-linking to a different hash. The motivation is recoverability — three weeks later, `git log` is now the only id needed to recall a commit's keypoints + stage telemetry. v0.60.0. Spec at `context/specs/19-ai-link-commit.md`.
- 2026-05-27 — **Unit 18 (`ai release`) code-complete** on `feat/agent-cli-improvements`. New `commitcraft ai release --version <vX.Y.Z> [--from <ref>] [--to <ref>]` subcommand. Same shape as `ai merge` but with `Type="RELEASE"`, `Scope=<version>`, defaults `--from`=most recent tag and `--to`=HEAD. Refactored shared helpers (`projectToReleaseCommits`, `serializeCommitRange`, `lastTagAt`) out of `merge.go` into `internal/cli/ai/range_helpers.go`. Storage caveat documented: TUI's release flow writes to a separate `releases` table; headless `ai release` writes to `commits` table so existing subcommands work unchanged. End-to-end smoke test produced draft id=1002 covering v0.55.0..HEAD (3 commits); verify caught the same `title_too_long_soft` warning as merge — release pipeline's title prompt is consistently verbose. Publishing (`ai release publish` wrapping `gh release create`) deferred to a follow-up. v0.59.0. Spec at `context/specs/18-ai-release.md`.
